...
if uploaded_file:
    ...
    ax.axhspan(30, smo2_start_max_norm, color='green', alpha=0.2, label='Zone 1 : Faible')
    ax.axhspan(10, 30, color='orange', alpha=0.2, label='Zone 2 : Modérée')
    ax.axhspan(0, 10, color='red', alpha=0.2, label='Zone 3 : Sévère')
    ...
    ax.set_title("SmO₂ – Analyse interactive avec zones d'intensité")
    ax.grid(True)
    ax.legend()
    st.pyplot(fig)
